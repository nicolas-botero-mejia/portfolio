...
|--------|-----------|----------------------|---------|
| **Primitive vs semantic** | Primitives in `primitiveTokens.ts` (colors, spacing, typography, radii, leading, tracking, shadow); semantics in `semanticTokens.ts` (themes light/dark). | DTCG: primitives as raw values; semantics as aliases/references to primitives; avoid binding UI to raw values. | Aligned. We use semantic roles (background, content, action, status) that map to primitives. |
